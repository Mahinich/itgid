...
function t7() {
    let randomIndex = Math.floor(Math.random() * a7.length);
    let randomChar = a7[randomIndex];

    document.querySelector('.out-7').textContent = randomChar;
}
...
function t10(event) {
    ...
    console.log(event);
    document.querySelector('.block-10').style.cssText = `
        height: ${h}px;
        width: ${w}px;
    `
}
...
function t11(event) {
    ...
    const keyboardItems = document.querySelectorAll('.keyboard');
    let key = event.key;
