...
function resolveF1(data) {
    ...
    if (data['random-number'] > 50) out.textContent = true
...
function f2() {
  const prms = new Promise((resolve, reject) => {
	...
	prms.then(resolveF2, rejectError);
}
...
function f8(){
  ...
  const select_1 = document.querySelector('.s-81').value;
  const select_2 = document.querySelector('.s-82').value;

  const requestHeaders = new Headers();
  requestHeaders.append('apikey', APIKEY);

  const promise_1 = new Promise(resolve => {
    fetch(`${URL}/api/27/sr/read/${select_1}`, {
      headers: requestHeaders
    })
    .then(response => resolve(response.json()))
  })
  ...
  const promise_2 = new Promise(resolve => {
    fetch(`${URL}/api/27/sr/read?race=${select_2}`, {
      headers: requestHeaders
    })
    .then(response => resolve(response.json()))
  })
  ...
  .then(data => {
    ...
    const imgFromS81 = document.createElement('img');
    imgFromS81.src = `${URL}${data[0].result.image}`;

    const imgFromS82 = document.createElement('img');
    imgFromS82.src = `${URL}${data[1].result.image}`;

    out.append(imgFromS81, imgFromS82);
  }) 
}
